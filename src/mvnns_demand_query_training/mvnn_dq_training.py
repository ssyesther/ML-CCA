...
from milps.gurobi_mip_mvnn_generic_v2 import GUROBI_MIP_MVNN_GENERIC_SINGLE_BIDDER_UTIL_MAX_V2 
...
def dq_train_mvnn_helper(model,
    ...
    for batch_idx, (demand_vector, price_vector) in enumerate(train_loader_demand_queries):
        ...
        if SATS_domain in ['GSVM', 'LSVM', 'MSVM']:
...
def dq_train_mvnn(SATS_auction_instance,
    ...
    if SATS_domain in ['GSVM', 'LSVM', 'MSVM']:
        ...
        expanded_input_dim = sum(capacity_generic_goods.values()) if isinstance(capacity_generic_goods, dict) else sum(capacity_generic_goods)
        print(f"[维度验证] 模型输入维度设置: {expanded_input_dim}")
        model = MVNN_GENERIC(input_dim=expanded_input_dim,
                    num_hidden_layers=num_hidden_layers,
                    num_hidden_units=num_hidden_units,
                    layer_type=layer_type,
                    target_max=target_max,
        ...
        print(f"[维度验证] 实际模型输入层维度: {model.layers[0].in_features}")
        assert model.layers[0].in_features == expanded_input_dim, "模型输入层维度设置失败"
    else:
        raise NotImplementedError(f'Unknown SATS domain: {SATS_domain}')
...
def dq_train_mvnn_parallel(bidder_id,# bidder_id must be first position for joblib.parallel!!
    ...
    print(f"Received parameters: {locals().keys()}")
    ...
    induced_values = []
    # 添加维度校验
    if P_train.shape != X_train.shape:
        error_msg = f"P_train和X_train维度不匹配: P_train.shape={P_train.shape}, X_train.shape={X_train.shape}"
    ...
    for i in range(len(P_train)):
        try:
            ...
            p = P_train[i].flatten()
            x = X_train[i].flatten()
            induced_values.append(np.dot(p, x))
        except ValueError as e:
            # 添加详细错误日志
            logging.error(f"样本{i}维度异常: P_train[{i}].shape={P_train[i].shape}, X_train[{i}].shape={X_train[i].shape}")
            raise ValueError(f"样本{i}点积计算失败: {str(e)}") from e
    ...
    if SATS_domain in ['GSVM', 'LSVM', 'MSVM']:
        ...
        expanded_input_dim = sum(capacity_generic_goods.values()) if isinstance(capacity_generic_goods, dict) else sum(capacity_generic_goods)
        print(f"[并行训练验证] 修正后的输入维度: {expanded_input_dim}")
        model = MVNN_GENERIC(input_dim=expanded_input_dim,
                    num_hidden_layers=num_hidden_layers,
                    num_hidden_units=num_hidden_units,
                    layer_type=layer_type,
