...
def init_demand_queries_mlca_increasing(SATS_auction_instance,
    ...
    all_goods = SATS_auction_instance.get_good_ids()  # 新增：获取所有商品ID
    good_index = {good: idx for idx, good in enumerate(all_goods)}  # 新增：创建索引映射
...
def init_demand_queries_mlca_cca(SATS_auction_instance,
    ...
    bidder_ids = SATS_auction_instance.get_bidder_ids()
    # 新增商品索引映射
    all_goods = SATS_auction_instance.get_good_ids()
    good_index = {good: idx for idx, good in enumerate(all_goods)}
    ...
    while bids_used < number_initial_bids:
        ...
        for bidder_id in bidder_ids:
            ...
            # 确保 bidder_demand 的长度与 total_demand 一致
            if len(bidder_demand) != len(total_demand):
                raise ValueError(f"Bidder {bidder_id} demand vector length ({len(bidder_demand)}) does not match total_demand length ({len(total_demand)}). "
                               f"Expected {m} items but got {len(bidder_demand)} items.")
