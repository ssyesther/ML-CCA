...
# Own Libs
from mlca_demand_queries.mlca_dq_util import key_to_int, format_solution_mip_new, init_demand_queries_mlca_unif, \
    timediff_d_h_m_s, init_demand_queries_mlca_increasing, init_demand_queries_mlca_cca
...
class MLCA_DQ_ECONOMIES:
    ...
    def __init__(self,
        ...
        self.good_indexs = set(SATS_auction_instance.get_good_indexs())
        self.M = len(self.good_ids)  # number of items
        self.Qinit = mechanism_parameters[
            'Qinit']  # number of intial data points, i.e., bundle-value pairs (queried randomly per bidder prior to the elicitation procedure, different per bidder)
        self.Qmax = mechanism_parameters[
            'Qmax']  # maximal number of possible value queries in the preference elicitation algorithm (PEA) per bidder
        ...
        if SATS_parameters['SATS_domain'] in ['LSVM', 'GSVM']:
            ...
            self.good_capacities = np.array([capacities_dictionary[good_id] for good_id in
                                             self.good_ids])  # self.good_capacities[j]: Capacity of good j
        ...
        self.max_linear_prices = {bidder_name: TRAIN_parameters[bidder_name]['max_linear_price'] for bidder_name in
                                  TRAIN_parameters.keys()}
        ...
        # Initial CCA per-round metrics (clock rounds 1..Qinit)
        self.initial_cca_efficiency_per_round = OrderedDict()
        self.initial_cca_scw_per_round = OrderedDict()
        # Optional: raised-bids variants for initial CCA clock rounds
        self.initial_cca_efficiency_raised_per_round = OrderedDict()
        self.initial_cca_scw_raised_per_round = OrderedDict()
        ...
        self.efficiency_per_iteration_profit_max = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for efficiency raised per auction round
        self.scw_per_iteration_profit_max = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for scw raised per auction round
        self.allocation_per_iteration_profit_max = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for allocation raised per auction round
        ...
        self.efficiency_per_iteration_profit_max_unraised = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for efficiency raised per auction round
        self.scw_per_iteration_profit_max_unraised = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for scw raised per auction round
        self.allocation_per_iteration_profit_max_unraised = {j: OrderedDict() for j in self.mechanism_parameters[
            'profit_max_grid']}  # storage for allocation raised per auction round
        self.final_allocation_profit_max_unraised = {j: OrderedDict() for j in
                                                     self.mechanism_parameters['profit_max_grid']}
        ...
        self.vcg_payments_profit_max = {j: OrderedDict(list(('Bidder_{}'.format(bidder_id), None) for bidder_id in
                                                            self.bidder_ids)) for j in
                                        self.mechanism_parameters['profit_max_grid']}
        self.vcg_payments_profit_max_unraised = {
            j: OrderedDict(list(('Bidder_{}'.format(bidder_id), None) for bidder_id in
        ...
        self.clearing_payments_profit_max = {j: OrderedDict(list(('Bidder_{}'.format(bidder_id), None) for bidder_id in
                                                                 self.bidder_ids)) for j in
                                             self.mechanism_parameters['profit_max_grid']}
        self.clearing_payments_profit_max_unraised = {
            j: OrderedDict(list(('Bidder_{}'.format(bidder_id), None) for bidder_id in
                                self.bidder_ids)) for j in self.mechanism_parameters['profit_max_grid']}
        self.clearing_payments = OrderedDict(list(('Bidder_{}'.format(bidder_id), None) for bidder_id in
                                                  self.bidder_ids))
        ...
        if self.wandb_tracking:
            merged_train_mvnn_dict = {bidder_name: {**TRAIN_parameters[bidder_name], **MVNN_parameters[bidder_name]} for
                                      bidder_name in TRAIN_parameters.keys()}
            try:
                ...
                logging.warning(f'wandb.init failed ({e}); falling back to offline or disabling tracking.')
                try:
                    ...
                    logging.warning(f'wandb offline init also failed ({e2}); disabling wandb tracking.')
            ...
            if self.wandb_tracking:
                ...
                if self.mechanism_parameters['calculate_profit_max_bids']:
                    for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                        wandb.define_metric(f'Efficiency Profit Max {number_of_profit_max_bids} Bids',
                ...
                if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                    for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                        wandb.define_metric(f'Efficiency Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)',
                                            step_metric="iterations")
                        wandb.define_metric(f'SCW Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)',
                                            step_metric="iterations")

                for profit_max_rounds in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
                    wandb.define_metric(f'Efficiency Profit Max ({profit_max_rounds} clock rounds)',
                                        step_metric="profit max bids")
                    wandb.define_metric(f'SCW Profit Max ({profit_max_rounds} clock rounds)', step_metric="profit max bids")
                    wandb.define_metric(f'Efficiency Profit Max ({profit_max_rounds} clock rounds) (unraised clock bids)',
                                        step_metric="profit max bids")
                    wandb.define_metric(f'SCW Profit Max ({profit_max_rounds} clock rounds) (unraised clock bids)',
                                        step_metric="profit max bids")
                    wandb.define_metric(f'WDP Relative Gap Profit Max ({profit_max_rounds} clock rounds)',
                                        step_metric="profit max bids")
                    wandb.define_metric(f'WDP Time Profit Max ({profit_max_rounds} clock rounds)',
                                        step_metric="profit max bids")
                    wandb.define_metric(f'WDP Unsatisfied Constraints Profit Max ({profit_max_rounds} clock rounds)',
                                        step_metric="profit max bids")
    ...
    def log_Qinit_efficiency(self):
        ...
        for clock_round in range(1, self.Qinit + 1):
            ...
            allocation, true_value, inferred_value, details = self.solve_WDP(inferred_bids, self.MIP_parameters,
                                                                             verbose=1)  # find the efficient allocation with respect to the inferred bids
            ...
            # Record initial CCA per-round metrics
            self.initial_cca_efficiency_per_round[clock_round] = efficiency
            self.initial_cca_scw_per_round[clock_round] = true_value
            ...
            for bidder_id in self.bidder_ids:
                ...
                if np.sum(round_matches) > 0:
                    last_round = np.max(np.where(round_matches)[
                                            0]) + 1  # because indexes start from 0, but clock rounds/iterations start from 1
                else:
            ...
            if self.mechanism_parameters['calculate_raised_bids']:
                ...
                allocation_raised, true_value_raised, inferred_value_raised, details_raised = self.solve_WDP(
                    raised_bids, self.MIP_parameters,
                    verbose=1)  # find the efficient allocation with respect to the inferred bids
                self.number_of_mips_solved['WDP'] += 1

                efficiency_raised = self.calculate_efficiency_of_allocation(allocation=allocation_raised,
                                                                            allocation_scw=true_value_raised)

                # Record raised-bids initial CCA metrics
                self.initial_cca_efficiency_raised_per_round[clock_round] = efficiency_raised
                self.initial_cca_scw_raised_per_round[clock_round] = true_value_raised
            ...
            # Only log to wandb if tracking is enabled
            if self.wandb_tracking:
    ...
    def extend_per_iteration_results(self):
        ...
        for future_iter in range(self.mlca_iteration + 1, (self.Qmax - self.Qinit) + 1):
            ...
            self.predicted_clearing_error_per_iteration[future_iter] = self.predicted_clearing_error_per_iteration[
                self.mlca_iteration]
            self.price_vector_per_iteration[future_iter] = self.price_vector_per_iteration[self.mlca_iteration]
            self.identical_price_vector_per_iteration[future_iter] = self.identical_price_vector_per_iteration[
                self.mlca_iteration]
            self.found_clearing_prices_per_iteration[future_iter] = self.found_clearing_prices_per_iteration[
                self.mlca_iteration]
            ...
            self.MIP_relative_gap_per_iteration[
                future_iter] = 0  # if we have found clearing prices -> we do not solve WDPs, set those metrics to 0
            ...
            self.unique_bundles_bid_on_per_iteration[future_iter] = self.unique_bundles_bid_on_per_iteration[
                self.mlca_iteration]

            if self.mechanism_parameters['calculate_raised_bids']:
                # if the market cleared (using the non-raised bids)
                # the raised results should be identical, i.e., we never even reach that phase.
                self.efficiency_per_iteration_raised_bids[future_iter] = self.efficiency_per_iteration[
                    self.mlca_iteration]
            ...
            if self.mechanism_parameters['calculate_profit_max_bids']:
                ...
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    self.efficiency_per_iteration_profit_max[number_of_profit_max_bids][future_iter] = \
                    self.efficiency_per_iteration[self.mlca_iteration]
                    self.scw_per_iteration_profit_max[number_of_profit_max_bids][future_iter] = self.scw_per_iteration[
                        self.mlca_iteration]
            ...
            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                ...
                # the profit max results should be identical, i.e., we never even reach that phase.
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    self.efficiency_per_iteration_profit_max_unraised[number_of_profit_max_bids][future_iter] = \
                    self.efficiency_per_iteration[self.mlca_iteration]
                    self.scw_per_iteration_profit_max_unraised[number_of_profit_max_bids][future_iter] = \
                    self.scw_per_iteration[self.mlca_iteration]
            ...
            if future_clock_round in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
                self.efficiency_per_iteration_profit_max_specific_round[future_clock_round] = [
                    self.efficiency_per_iteration[self.mlca_iteration] for _ in
                    range(self.mechanism_parameters['profit_max_grid'][-1] + 1)]
                self.efficiency_per_iteration_profit_max_unraised_specific_round[future_clock_round] = [
                    self.efficiency_per_iteration[self.mlca_iteration] for _ in
                    range(self.mechanism_parameters['profit_max_grid'][-1] + 1)]
                self.MIP_relative_gap_per_iteration_profit_max_specific_round[future_clock_round] = [0 for _ in range(
                    self.mechanism_parameters['profit_max_grid'][-1] + 1)]
                self.MIP_time_per_iteration_profit_max_specific_round[future_clock_round] = [0 for _ in range(
                    self.mechanism_parameters['profit_max_grid'][-1] + 1)]
                self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[future_clock_round] = [0 for _
                                                                                                                in
                                                                                                                range(
                ...
                self.scw_per_iteration_profit_max_specific_round[future_clock_round] = [
                    self.scw_per_iteration[self.mlca_iteration] for _ in
                    range(self.mechanism_parameters['profit_max_grid'][-1] + 1)]
                self.scw_per_iteration_profit_max_unraised_specific_round[future_clock_round] = [
                    self.scw_per_iteration[self.mlca_iteration] for _ in
                    range(self.mechanism_parameters['profit_max_grid'][-1] + 1)]

            logging.warning(f'Extending wandb logged results for iteration {future_iter}')
            if self.wandb_tracking:
                price_dict_to_save = {f'Price Good {i}': self.price_vector_per_iteration[future_iter][i] for i in
                                      self.good_indexs}
                wandb_dict = {"Efficiency Clock Bids": self.efficiency_per_iteration[future_iter],
                              ...
                              "WDP Unsatisfied Constraints Clock Bids": self.MIP_unsatisfied_constraints_per_iteration[
                                  future_iter],
                              ...
                              "Identical Price Vector": 1 if self.identical_price_vector_per_iteration[
                                  future_iter] else 0,
                              ...
                              "Found Clearing Prices": 1 if self.found_clearing_prices_per_iteration[
                                  future_iter] else 0,
                ...
                unique_bundles_bid_on_dict_to_save = {
                    f'Bidder {bidder_id} Unique Bundles Bid On': self.unique_bundles_bid_on_per_iteration[future_iter][
                        f'Bidder_{bidder_id}'] for bidder_id in self.bidder_ids}
                wandb_dict.update(unique_bundles_bid_on_dict_to_save)

                if self.mechanism_parameters['calculate_raised_bids']:
                    wandb_dict.update(
                        {"Efficiency Raised Clock Bids": self.efficiency_per_iteration_raised_bids[future_iter],
                         "Efficiency Raised Clock Bids Per Clock Round": self.efficiency_per_iteration_raised_bids[
                             future_iter],
                         "WDP Relative Gap Raised Clock Bids": self.MIP_relative_gap_per_iteration_raised_bids[
                             future_iter],
                         "WDP Time Raised Clock Bids": self.MIP_time_per_iteration_raised_bids[future_iter],
                         "WDP Unsatisfied Constraints Raised Clock Bids":
                             self.MIP_unsatisfied_constraints_per_iteration_raised_bids[future_iter],
                         "SCW Raised Bids": self.scw_per_iteration_raised_bids[future_iter]})

                if self.mechanism_parameters['calculate_profit_max_bids']:
                    for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                        wandb_dict.update({f'Efficiency Profit Max {number_of_profit_max_bids} Bids':
                                               self.efficiency_per_iteration_profit_max[number_of_profit_max_bids][
                                                   future_iter],
                                           f'SCW Profit Max {number_of_profit_max_bids} Bids':
                                               self.scw_per_iteration_profit_max[number_of_profit_max_bids][
                                                   future_iter]})

                if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                    for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                        wandb_dict.update({
                                              f'Efficiency Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)':
                                                  self.efficiency_per_iteration_profit_max_unraised[
                                                      number_of_profit_max_bids][future_iter],
                                              f'SCW Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)':
                                                  self.scw_per_iteration_profit_max_unraised[number_of_profit_max_bids][
                                                      future_iter]})
                ...
                if future_clock_round in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
                    for number_of_profit_max_bids in range(self.mechanism_parameters['profit_max_grid'][-1] + 1):
                        profit_max_dict_round = {
                            f'Efficiency Profit Max ({future_clock_round} clock rounds)':
                                self.efficiency_per_iteration_profit_max_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                            f'WDP Relative Gap Profit Max ({future_clock_round} clock rounds)':
                                self.MIP_relative_gap_per_iteration_profit_max_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                            f'WDP Time Profit Max ({future_clock_round} clock rounds)':
                                self.MIP_time_per_iteration_profit_max_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                            f'WDP Unsatisfied Constraints Profit Max ({future_clock_round} clock rounds)':
                                self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[
                                    future_clock_round][number_of_profit_max_bids],
                            f'SCW Profit Max ({future_clock_round} clock rounds)':
                                self.scw_per_iteration_profit_max_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                            f'Efficiency Profit Max ({future_clock_round} clock rounds) (unraised clock bids)':
                                self.efficiency_per_iteration_profit_max_unraised_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                            f'SCW Profit Max ({future_clock_round} clock rounds) (unraised clock bids)':
                                self.scw_per_iteration_profit_max_unraised_specific_round[future_clock_round][
                                    number_of_profit_max_bids],
                ...
                for j in range(self.get_number_of_elicited_dqs()[0] + 1, self.Qmax + 1):
                    extended_price_dict_to_save = {f'Extended Prices Good {i}': self.elicited_dqs['Bidder_0'][1][
                        self.get_number_of_elicited_dqs()[0] - 1, i] for i in self.good_indexs}
                    extended_price_dict_to_save["Number of Elicited Bids"] = j
                    wandb.log(extended_price_dict_to_save)

    def wandb_final_table(self):
        columns = ["SATS Seed", "Efficiency in %", "Relative (VCG) Revenue in %", "Clearing Error",
                   "Total Time Elapsed (sec)"]
        data = [self.SATS_auction_instance_seed, self.final_allocation_efficiency, self.relative_revenue,
                self.clearing_error_per_iteration[len(self.clearing_error_per_iteration)],
        ...
        if self.mechanism_parameters['calculate_profit_max_bids']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                columns = columns + [f"Efficiency Profit Max {number_of_profit_max_bids} Bids in %",
                                     f"Relative (VCG) Revenue Profit Max {number_of_profit_max_bids} Bids in %"]
                data = data + [self.final_allocation_efficiency_profit_max[number_of_profit_max_bids],
                               self.relative_revenue_profit_max[number_of_profit_max_bids]]

        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                columns = columns + [
                    f"Efficiency Profit Max {number_of_profit_max_bids} Bids (unraised clock bids) in %",
                    f"Relative (VCG) Revenue Profit Max {number_of_profit_max_bids} Bids (unraised clock bids) in %"]
                data = data + [self.final_allocation_efficiency_profit_max_unraised[number_of_profit_max_bids],
                               self.relative_revenue_profit_max_unraised[number_of_profit_max_bids]]
    ...
    def save_results(self,
        ...
        # 注意：不再在 results.json 中记录初始 CCA 的每轮效率/SCW，改为单独文件 initial_cca_result.json。
        ...
        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            save_dict[
                'Efficiency per Iteration Profit Max Unraised'] = self.efficiency_per_iteration_profit_max_unraised
        ...
        if self.wandb_tracking and not no_wandb_logging:
            ...
            if self.mlca_iteration == 1:
                ...
                wandb_init_dict = {"Efficiency Clock Bids": self.efficiency_per_iteration[0],
                                   ...
                                   "WDP Unsatisfied Constraints Clock Bids":
                                       self.MIP_unsatisfied_constraints_per_iteration[0],
                ...
                if self.mechanism_parameters['calculate_raised_bids']:
                    ...
                    wandb_init_dict["Efficiency Raised Clock Bids Per Clock Round"] = \
                    self.efficiency_per_iteration_raised_bids[0]
                    wandb_init_dict["WDP Relative Gap Raised Clock Bids"] = \
                    self.MIP_relative_gap_per_iteration_raised_bids[0]
                    wandb_init_dict["WDP Time Raised Clock Bids"] = self.MIP_time_per_iteration_raised_bids[0]
                    wandb_init_dict["WDP Unsatisfied Constraints Raised Clock Bids"] = \
                    self.MIP_unsatisfied_constraints_per_iteration_raised_bids[0]
                    wandb_init_dict["SCW Raised Bids"] = self.scw_per_iteration_raised_bids[0]

                if self.mechanism_parameters['calculate_profit_max_bids']:
                    for j in self.mechanism_parameters['profit_max_grid']:
                        wandb_init_dict[f"Efficiency Profit Max {j} Bids"] = \
                        self.efficiency_per_iteration_profit_max[j][0]
                        wandb_init_dict[f"SCW Profit Max {j} Bids"] = self.scw_per_iteration_profit_max[j][0]

                if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                    # set_trace()
                    for j in self.mechanism_parameters['profit_max_grid']:
                        wandb_init_dict[f"Efficiency Profit Max {j} Bids (unraised clock bids)"] = \
                        self.efficiency_per_iteration_profit_max_unraised[j][0]
                        wandb_init_dict[f"SCW Profit Max  {j} Bids (unraised clock bids)"] = \
                        self.scw_per_iteration_profit_max_unraised[j][0]

                unique_bundles_bid_on_dict_to_save = {
                    f'Bidder {bidder_id} Unique Bundles Bid On': self.unique_bundles_bid_on_per_iteration[0][
                        f'Bidder_{bidder_id}'] for bidder_id in self.bidder_ids}
                ...
                for j in range(1, self.get_number_of_elicited_dqs()[0]):
                    extended_price_dict_to_save = {
                        f'Extended Prices Good {i}': self.elicited_dqs['Bidder_0'][1][j - 1, i] for i in
                        self.good_indexs}  # NOTE: only works for increasing prices/CCA
                    extended_price_dict_to_save["Number of Elicited Bids"] = j
                    wandb.log(extended_price_dict_to_save)
            ...
            if clock_round in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
                for number_of_profit_max_bids in range(self.mechanism_parameters['profit_max_grid'][-1] + 1):
                    profit_max_dict_round = {
                        f'Efficiency Profit Max ({clock_round} clock rounds)':
                            self.efficiency_per_iteration_profit_max_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'SCW Profit Max ({clock_round} clock rounds)':
                            self.scw_per_iteration_profit_max_specific_round[clock_round][number_of_profit_max_bids],
                        f'Efficiency Profit Max ({clock_round} clock rounds) (unraised clock bids)':
                            self.efficiency_per_iteration_profit_max_unraised_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'SCW Profit Max ({clock_round} clock rounds) (unraised clock bids)':
                            self.scw_per_iteration_profit_max_unraised_specific_round[clock_round][
                                number_of_profit_max_bids],
                        'profit max bids': number_of_profit_max_bids,
                    }
                    wandb.log(profit_max_dict_round)
            ...
            price_dict_to_save = {f'Price Good {i}': self.price_vector_per_iteration[self.mlca_iteration][i] for i in
                                  self.good_indexs}

            # create a dictionary with the round in which each agent bid on the bundle they won
            winning_round_bids = {}
            for bidder_id in self.bidder_ids:
                bundle_won = self.allocation_per_iteration[self.mlca_iteration][f'Bidder_{bidder_id}'][
                    'allocated_bundle']
                bundle_won = np.array(bundle_won)
            ...
            if self.mechanism_parameters['calculate_raised_bids']:
                wandb_dict["Efficiency Raised Clock Bids"] = self.efficiency_per_iteration_raised_bids[
                    self.mlca_iteration]
                wandb_dict["Efficiency Raised Clock Bids Per Clock Round"] = self.efficiency_per_iteration_raised_bids[
                    self.mlca_iteration]
                wandb_dict["WDP Relative Gap Raised Clock Bids"] = self.MIP_relative_gap_per_iteration_raised_bids[
                    self.mlca_iteration]
                wandb_dict["WDP Time Raised Clock Bids"] = self.MIP_time_per_iteration_raised_bids[self.mlca_iteration]
                wandb_dict["WDP Unsatisfied Constraints Raised Clock Bids"] = \
                self.MIP_unsatisfied_constraints_per_iteration_raised_bids[self.mlca_iteration]
                wandb_dict["SCW Raised Bids"] = self.scw_per_iteration_raised_bids[self.mlca_iteration]

            if self.mechanism_parameters['calculate_profit_max_bids']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    wandb_dict[f'Efficiency Profit Max {number_of_profit_max_bids} Bids'] = \
                    self.efficiency_per_iteration_profit_max[number_of_profit_max_bids][self.mlca_iteration]
                    wandb_dict[f'SCW Profit Max {number_of_profit_max_bids} Bids'] = \
                    self.scw_per_iteration_profit_max[number_of_profit_max_bids][self.mlca_iteration]

            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    wandb_dict[f'Efficiency Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)'] = \
                    self.efficiency_per_iteration_profit_max_unraised[number_of_profit_max_bids][self.mlca_iteration]
                    wandb_dict[f'SCW Profit Max {number_of_profit_max_bids} Bids (unraised clock bids)'] = \
                    self.scw_per_iteration_profit_max_unraised[number_of_profit_max_bids][self.mlca_iteration]

            wandb_dict.update(price_dict_to_save)

            print('Current iteration: ', self.mlca_iteration)
            # set_trace()
            unique_bundles_bid_on_dict_to_save = {f'Bidder {bidder_id} Unique Bundles Bid On':
            ...
            extended_price_dict_to_save = {f'Extended Prices Good {i}': self.elicited_dqs['Bidder_0'][1][
                self.get_number_of_elicited_dqs()[0] - 1, i] for i in
                                           self.good_indexs}  # NOTE: only works for increasing prices/cca
                ...
                for number_of_profit_max_bids in range(self.mechanism_parameters['profit_max_grid'][-1] + 1):
                    profit_max_dict_round = {
                        f'Efficiency Profit Max ({clock_round} clock rounds)':
                            self.efficiency_per_iteration_profit_max_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'WDP Relative Gap Profit Max ({clock_round} clock rounds)':
                            self.MIP_relative_gap_per_iteration_profit_max_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'WDP Time Profit Max ({clock_round} clock rounds)':
                            self.MIP_time_per_iteration_profit_max_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'WDP Unsatisfied Constraints Profit Max ({clock_round} clock rounds)':
                            self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[clock_round][
                                number_of_profit_max_bids],
                        f'SCW Profit Max ({clock_round} clock rounds)':
                            self.scw_per_iteration_profit_max_specific_round[clock_round][number_of_profit_max_bids],
                        f'Efficiency Profit Max ({clock_round} clock rounds) (unraised clock bids)':
        ...
        # 在首次进入 MLCA（迭代==1）且 init_method 为 'cca' 时，
        # 生成独立的 initial_cca_result.json 文件，仅记录初始 CCA 的钟轮 [1..Qinit]
        if (self.mlca_iteration == 1) and (self.mechanism_parameters.get('initial_demand_query_method') == 'cca') and (self.Qinit > 0):
            try:
                self.save_initial_cca_results(res_path)
            except Exception as e:
                logging.warning(f'Failed to save initial_cca_result.json: {e}')

        json.dump(save_dict, open(os.path.join(res_path, 'results.json'), 'w'), indent=4,
                  sort_keys=False, separators=(', ', ': '),
                  ensure_ascii=False, cls=NumpyEncoder)

    def save_initial_cca_results(self, res_path):
        """
        生成初始 CCA 阶段（Qinit 钟轮）每轮的投标快照，并基于累积的 inferred bids 求解 WDP，
        输出每轮的效率（efficiency）与社会福利（SCW）到 initial_cca_result.json。

        - 每轮 bids 记录：对每个投标者，记录该轮的需求向量 `X` 与价格向量 `P`（该轮价格）。
        - WDP 计算：使用 calculate_inferred_bids_per_round(r) 的累积去重 bids，调用 solve_WDP，
          得到 allocation、true_value（SCW）、inferred_value；效率基于 SATS 最优 SCW 计算。
        """
        from collections import OrderedDict
        result = OrderedDict()
        result['SATS Seed'] = self.SATS_auction_instance_seed
        result['Qinit'] = int(self.Qinit)
        rounds = []

        for r in range(1, int(self.Qinit) + 1):
            # 记录该轮的每个投标者的投标（需求与当轮价格）
            bids_round = {}
            for bidder in self.bidder_names:
                X_row = self.elicited_dqs[bidder][0][r - 1]
                P_row = self.elicited_dqs[bidder][1][r - 1]
                bids_round[bidder] = {
                    'X': X_row,
                    'P': P_row
                }

            # 当轮价格向量（各投标者相同，取任意一个即可）
            price_vector = self.elicited_dqs[self.bidder_names[0]][1][r - 1]

            # 累积到第 r 轮的 inferred bids（去重），并用 WDP 求解效率与SCW
            inferred_bids = self.calculate_inferred_bids_per_round(r)
            allocation, true_value, inferred_value, details = self.solve_WDP(
                elicited_bids=inferred_bids,
                MIP_parameters=self.MIP_parameters,
                verbose=0
            )
            efficiency = self.calculate_efficiency_of_allocation(
                allocation=allocation,
                allocation_scw=true_value
            )
            ...
            rounds.append({
                ...
                'scw': true_value,
                'wdp_inferred_value': inferred_value,
                'bids': bids_round
            })

        result['clock_rounds'] = rounds

        json.dump(
            result,
            open(os.path.join(res_path, 'initial_cca_result.json'), 'w'),
            indent=4,
            sort_keys=False,
            separators=(', ', ': '),
            ensure_ascii=False,
    ...
    def get_info(self,
        ...
        if final_summary:
            ...
            logging.warning(
                f'WDP MIP TIME: {self.total_time_elapsed_distr["WDP MIP"][0]} ({100 * self.total_time_elapsed_distr["WDP MIP"][1]}%) | Price Generation TIME: {self.total_time_elapsed_distr["Price Vector Generation"][0]} ({100 * self.total_time_elapsed_distr["Price Vector Generation"][1]}%) | ML TRAIN TIME: {self.total_time_elapsed_distr["ML TRAIN"][0]} ({100 * self.total_time_elapsed_distr["ML TRAIN"][1]}%) | OTHER TIME: {self.total_time_elapsed_distr["OTHER"][0]} ({100 * self.total_time_elapsed_distr["OTHER"][1]}%)')
    ...
    def calculate_inferred_bids(self):
        ...
        for bidder in self.bidder_names:
            ...
            V_unique = np.array([np.max(V[np.where((X == X_unique[i]).all(axis=1))[0]]) for i in
                                 range(X_unique.shape[0])])  # tell Haskell I said hi.

            inferred_bids[bidder] = [X_unique, V_unique]
            print(f'For bidder {bidder} inferred bid after removing duplicates is : {inferred_bids[bidder]}')
    ...
    def raise_inferred_bids(self):
        ...
        for bidder_id in range(len(self.bidder_names)):
            ...
            for i in range(X_unique.shape[0]):
                raised_bid = self.SATS_auction_instance.calculate_value(bidder_id=bidder_id, bundle=X_unique[i])
                V.append(raised_bid)
    ...
    def calculate_inferred_bids_per_round(self, round_number):
        ...
        for bidder in self.bidder_names:
            ...
            # set_trace()
            print(
                f'For bidder {bidder} inferred bid shape up to round {round_number} after removing duplicates is : {inferred_bids[bidder][0].shape}')
    ...
    def raise_inferred_bids_per_round(self, round_number):
        ...
        for bidder_id in range(len(self.bidder_names)):
            ...
            bundles = [
                [all_items[i] for i, val in enumerate(vec) if val == 1]
                for vec in X
            ]
            # for every bidder, for all of her unique inferred bids, calculate the value of that bundle
            for i in range(X.shape[0]):
                raised_bid = self.SATS_auction_instance.calculate_value(bidder_id, bundles[i])
                V.append(raised_bid)
    ...
    def calculate_profit_max_bids(self):
        ...
        for bidder_id in range(len(self.bidder_names)):
            ...
            demand_response_total = self.SATS_auction_instance.get_best_bundles(bidder_id, price_vector,
                                                                                self.mechanism_parameters[
                                                                                    'profit_max_grid'][-1])
    ...
    def calculate_efficiency_per_iteration(self):
        ...
        allocation, true_value, inferred_value, details = self.solve_WDP(self.inferred_bids, self.MIP_parameters,
                                                                         verbose=1)  # find the efficient allocation with respect to the inferred bids
        self.number_of_mips_solved['WDP'] += 1
        ...
        if self.mechanism_parameters['calculate_raised_bids']:
            ...
            allocation_raised, true_value_raised, inferred_value_raised, details = self.solve_WDP(
                self.inferred_bids_raised, self.MIP_parameters,
                verbose=1)  # find the efficient allocation with respect to the raised bids
            self.number_of_mips_solved['WDP'] += 1
            ...
            self.MIP_unsatisfied_constraints_per_iteration_raised_bids[self.mlca_iteration] = details[
        ...
        if self.mechanism_parameters['calculate_profit_max_bids'] or self.mechanism_parameters[
            'calculate_profit_max_bids_unraised'] or current_clock_round in self.mechanism_parameters[
            ...
            if self.mechanism_parameters['calculate_profit_max_bids']:
                for profit_max_bids_number in self.mechanism_parameters['profit_max_grid']:
                    ...
                    total_bids = {bidder_name: [np.concatenate(
                        (initial_bids[bidder_name][0], self.profit_max_bids[bidder_name][0][:profit_max_bids_number])),
                                                np.concatenate((initial_bids[bidder_name][1],
                                                                self.profit_max_bids[bidder_name][1][
                                                                :profit_max_bids_number]))] for bidder_name in
                                  self.bidder_names}

                    self.profit_max_bids_combined[profit_max_bids_number] = copy.deepcopy(
                        total_bids)  # store a copy of the combined bids for final payment calculation

                    # calculate the efficiency for the profit-max bids
                    allocation_profit_max, true_value_profit_max, inferred_value_profit_max, details = self.solve_WDP(
                        total_bids, self.MIP_parameters, verbose=1)
                    self.number_of_mips_solved['WDP'] += 1

                    self.allocation_per_iteration_profit_max[profit_max_bids_number][
                        self.mlca_iteration] = allocation_profit_max
                    efficiency_profit_max = self.calculate_efficiency_of_allocation(allocation=allocation_profit_max,
                                                                                    allocation_scw=true_value_profit_max)
                    self.efficiency_per_iteration_profit_max[profit_max_bids_number][
                        self.mlca_iteration] = efficiency_profit_max
                    logging.info(
                        f'Current efficiency (profit max bids, {profit_max_bids_number}): {efficiency_profit_max}')
                    # set_trace()

                    self.scw_per_iteration_profit_max[profit_max_bids_number][
                        self.mlca_iteration] = true_value_profit_max

            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                for profit_max_bids_number in self.mechanism_parameters['profit_max_grid']:
                    # combine the right number of profit max bids with the the (raised) clock bids

                    total_bids_unraised = {bidder_name: [np.concatenate((initial_bids_unraised[bidder_name][0],
                                                                         self.profit_max_bids[bidder_name][0][
                                                                         :profit_max_bids_number])), np.concatenate((
                                                                                                                    initial_bids_unraised[
                                                                                                                        bidder_name][
                                                                                                                        1],
                                                                                                                    self.profit_max_bids[
                                                                                                                        bidder_name][
                                                                                                                        1][
                                                                                                                    :profit_max_bids_number]))]
                                           for bidder_name in self.bidder_names}

                    self.profit_max_bids_combined_unraised[profit_max_bids_number] = copy.deepcopy(
                        total_bids)  # store a copy of the combined bids for final payment calculation

                    # calculate the efficiency for the profit-max bids
                    allocation_profit_max_unraised, true_value_profit_max_unraised, inferred_value_profit_max_unraised, details = self.solve_WDP(
                        total_bids_unraised, self.MIP_parameters, verbose=1)
                    self.number_of_mips_solved['WDP'] += 1

                    self.allocation_per_iteration_profit_max_unraised[profit_max_bids_number][
                        self.mlca_iteration] = allocation_profit_max_unraised
                    efficiency_profit_max_unraised = self.calculate_efficiency_of_allocation(
                        allocation=allocation_profit_max_unraised, allocation_scw=true_value_profit_max_unraised)
                    self.efficiency_per_iteration_profit_max_unraised[profit_max_bids_number][
                        self.mlca_iteration] = efficiency_profit_max_unraised
                    logging.info(
                        f'Current efficiency (profit max bids (unraised clock bids), {profit_max_bids_number}): {efficiency_profit_max_unraised}')
                    # set_trace()

                    self.scw_per_iteration_profit_max_unraised[profit_max_bids_number][
                        self.mlca_iteration] = true_value_profit_max_unraised
        ...
        if current_clock_round in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
            ...
            self.efficiency_per_iteration_profit_max_specific_round[current_clock_round] = [
                self.efficiency_per_iteration_raised_bids[self.mlca_iteration]]
            self.efficiency_per_iteration_profit_max_unraised_specific_round[current_clock_round] = [
                self.efficiency_per_iteration[self.mlca_iteration]]

            self.scw_per_iteration_profit_max_specific_round[current_clock_round] = [
                self.scw_per_iteration_raised_bids[self.mlca_iteration]]
            self.scw_per_iteration_profit_max_unraised_specific_round[current_clock_round] = [
                self.scw_per_iteration[self.mlca_iteration]]

            self.MIP_relative_gap_per_iteration_profit_max_specific_round[current_clock_round] = [
                self.MIP_relative_gap_per_iteration_raised_bids[self.mlca_iteration]]
            self.MIP_time_per_iteration_profit_max_specific_round[current_clock_round] = [
                self.MIP_time_per_iteration_raised_bids[self.mlca_iteration]]
            self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[current_clock_round] = [
                self.MIP_unsatisfied_constraints_per_iteration_raised_bids[self.mlca_iteration]]
            ...
            for profit_max_bids_number in range(1, self.mechanism_parameters['profit_max_grid'][-1] + 1):
                ...
                efficiency_profit_max = self.calculate_efficiency_of_allocation(allocation=allocation_profit_max,
                                                                                allocation_scw=true_value_profit_max)
                self.efficiency_per_iteration_profit_max_specific_round[current_clock_round].append(
                    efficiency_profit_max)
                self.scw_per_iteration_profit_max_specific_round[current_clock_round].append(true_value_profit_max)
                self.MIP_relative_gap_per_iteration_profit_max_specific_round[current_clock_round].append(
                    details['Relative Gap'])
                self.MIP_time_per_iteration_profit_max_specific_round[current_clock_round].append(details['Time'])
                self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[current_clock_round].append(
                    details['Unsatisfied Constraints'])

                logging.info(
                    f'Current efficiency clock round {current_clock_round}, {profit_max_bids_number} profit max bids: {efficiency_profit_max}')
                ...
                if profit_max_bids_number in self.mechanism_parameters['profit_max_grid']:
                    self.profit_max_bids_specific_round[profit_max_bids_number] = copy.deepcopy(
                        total_bids)  # store a copy of the combined bids for final payment calculation
                    self.profit_max_bids_unraised_specific_round[profit_max_bids_number] = copy.deepcopy(
                        total_bids_unraised)  # store a copy of the combined bids for final payment calculation
                ...
                efficiency_profit_max_unraised = self.calculate_efficiency_of_allocation(
                    allocation=allocation_profit_max_unraised, allocation_scw=true_value_profit_max_unraised)
                self.efficiency_per_iteration_profit_max_unraised_specific_round[current_clock_round].append(
                    efficiency_profit_max_unraised)
                self.scw_per_iteration_profit_max_unraised_specific_round[current_clock_round].append(
                    true_value_profit_max_unraised)

                end = datetime.now()
                logging.info(
                    f'Current efficiency (profit max bids (unraised clock bids), {profit_max_bids_number}: {efficiency_profit_max_unraised}')
                logging.info(
                    f'Calculating both WDPs for {profit_max_bids_number} profit-max bids took {end - start} seconds')
    ...
    def set_initial_dqs(self,
        ...
        if method == 'random':
            ...
            self.elicited_dqs = init_demand_queries_mlca_unif(SATS_auction_instance=self.SATS_auction_instance,
                                                              ...
                                                              max_linear_prices={k: self.TRAIN_parameters[k][
                                                                                        'max_linear_prices_multiplier'] * v
                                                                                 for k, v in
                                                                                 self.max_linear_prices.items()},
                                                              seed=self.SATS_auction_instance_seed,
                                                              include_null_price=True,
                                                              bidder_id=None
                                                              # if not None: only generate data for the present bidder ids
                                                              )
            ...
            self.elicited_dqs = init_demand_queries_mlca_increasing(SATS_auction_instance=self.SATS_auction_instance,
                                                                    number_initial_bids=self.Qinit,
                                                                    start_linear_item_prices=
                                                                    self.TRAIN_parameters[self.bidder_names[0]][
                                                                        'start_linear_item_prices'],
                                                                    # NOTE: same for all bidders
                                                                    end_linear_item_prices=
                                                                    self.TRAIN_parameters[self.bidder_names[0]][
                                                                        'end_linear_item_prices'],
                                                                    # NOTE: same for all bidders
                                                                    bidder_id=None
            ...
            self.elicited_dqs = init_demand_queries_mlca_cca(SATS_auction_instance=self.SATS_auction_instance,
                                                             ...
                                                             number_initial_bids=self.Qinit,
                                                             start_linear_item_prices=self.mechanism_parameters[
                                                                                          'cca_start_linear_item_prices'] *
                                                                                      self.mechanism_parameters[
                                                                                          'cca_initial_prices_multiplier'],
    ...
    def solve_SATS_auction_instance(self):
        ...
        logging.debug('Optimal Allocation: %s', self.SATS_auction_instance_allocation)
        logging.debug('Optimal SCW: %s', self.SATS_auction_instance_scw)
    ...
    def estimation_step(self):
        ...
        if self.dynamic_scaling:
            ...
            for bidder in self.bidder_names:
                logging.info(
                    f'Ratio of new scale to old scale for bidder {bidder}: {new_scales[bidder] / self.scales[bidder]}')
        ...
        if self.parallelize_training:
            m = Parallel(n_jobs=-1)(
                delayed(dq_train_mvnn_parallel)(
                    bidder_id=bidder_id,
                    ...
                    MIP_parameters=self.MIP_parameters,
                    GSVM_national_bidder_goods_of_interest=self.SATS_parameters[
                        'GSVM_national_bidder_goods_of_interest'],
                    wandb_tracking=False,
                    num_cpu_per_job=1,
                    SATS_auction_instance=self.SATS_auction_instance  # 新增关键参数
                ) for bidder_id in self.bidder_ids  # 直接遍历bidder_ids
            )
            ...
            for bidder, seed in zip(self.bidder_names, training_seeds):
                ...
                model, train_metrics = dq_train_mvnn(SATS_auction_instance=self.SATS_auction_instance,
                                                     capacity_generic_goods=self.good_capacities,
                                                     P_train=self.elicited_dqs[bidder][1] / self.scales[bidder],
                                                     # NOTE: method requires scaled P_train! scale the data to the range [0,1]
    ...
    def generate_dq(self):
        ...
        start = datetime.now()
        if self.mechanism_parameters["new_query_option"] in ["gd_linear_prices_on_W", "gd_linear_prices_on_W_v2",
            ...
            if self.mechanism_parameters["new_query_option"] == "gd_linear_prices_on_W":
                ...
                price_vector, predicted_ce, mips_solved = minimize_W_v2(bidder_models,
                                                                        ...
                                                                        # does not start from 0 prices, but instead from the prices of the last iteration.
                                                                        capacities=self.good_capacities,
                                                                        scale=scale_list,
                                                                        SATS_domain=self.SATS_parameters['SATS_domain'],
                                                                        GSVM_national_bidder_goods_of_interest=
                                                                        ...
                                                                        self.mechanism_parameters[
                                                                            'W_v2_max_steps_without_improvement'],
                                                                        lr=self.mechanism_parameters['W_v2_lr'],
                                                                        lr_decay=self.mechanism_parameters[
                                                                            'W_v2_lr_decay'],
                                                                        MIP_parameters=self.MIP_parameters,
                                                                        )
                ...
                price_vector, predicted_ce, mips_solved, all_Ws, all_CEs, all_CEs_norm_1, is_feasible = minimize_W_v3(
                    bidder_models,
                    starting_price_vector=starting_price_vector,
                    capacities=self.good_capacities,
                    scale=scale_list,
                    ...
                    feasibility_multiplier_increase_factor=self.mechanism_parameters[
                        'W_v3_feasibility_multiplier_increase_factor'],
                ...
                self.perturbed_prices = False
                previous_price_vectors = self.elicited_dqs['Bidder_0'][1][
                                         1:]  # so that we exclude the prive vector at zero prices.
                ...
                iteration_threshold = self.mechanism_parameters['identical_p_threshold'] * (
                            self.mechanism_parameters['identical_p_threshold_decay'] ** self.mlca_iteration)
                ...
                while min(relative_differences_l_infty) < iteration_threshold:
                    ...
                    price_vector = np.random.uniform(low=max(0, (1 - randomness_scale)) * price_vector_copy,
                                                     high=(1 + randomness_scale) * price_vector_copy)
                    absolute_differences = np.abs(previous_price_vectors - price_vector)
                    relative_differences = absolute_differences / previous_price_vectors
                    relative_differences_l_infty = np.max(relative_differences, axis=1)
                ...
                if (
                        self.mlca_iteration % 10 == 1) and self.wandb_tracking:  # we start counting iterations from 1, not form 0.
                    ...
                    for i in range(len(all_Ws)):
                        wandb.log({f'W per GD step iteration {self.mlca_iteration}': all_Ws[i],
                                   f'CE per GD step iteration {self.mlca_iteration}': all_CEs[i],
                                   f'CE norm 1 per GD step iteration {self.mlca_iteration}': all_CEs_norm_1[i],
                                   "GD Step": i})
            ...
            price_vector, predicted_ce, mips_solved, all_Ws, all_CEs, all_CEs_norm_1 = minimize_W_cheating(
                self.SATS_auction_instance,
                self.bidder_ids,
                starting_price_vector=starting_price_vector,
                capacities=self.good_capacities,
                max_steps_without_improvement=self.mechanism_parameters['W_v3_max_steps_without_improvement'],
        ...
        for bidder_id in self.bidder_ids:
            try:
                demand_response = np.array(
                    self.SATS_auction_instance.get_best_bundles(bidder_id, price_vector, 1)[0])  # convert to np array
                print(f'demand_response: {demand_response}')
            except:
                demand_response = self.elicited_dqs[f'Bidder_{bidder_id}'][0][
                    -1].copy()  # if the bidder does not respond, use the last demand response
            ...
            self.elicited_dqs[f'Bidder_{bidder_id}'][1] = np.concatenate(
                (self.elicited_dqs[f'Bidder_{bidder_id}'][1], price_vector.reshape(1, -1)), axis=0)
            self.elicited_dqs[f'Bidder_{bidder_id}'][0] = np.concatenate(
                (self.elicited_dqs[f'Bidder_{bidder_id}'][0], demand_response.reshape(1, -1)), axis=0)

        over_demand = real_demand - self.good_capacities
        # real_ce = np.sum(over_demand ** 2)
        real_ce = float(np.sum(np.maximum(over_demand, 0.0)))
    ...
    def calculate_final_allocation(self):
        ...
        if self.mechanism_parameters['calculate_raised_bids']:
            ...
            start = datetime.now()
            allocation_raised_bids, true_value_raised_bids, inferred_value_raised_bids, details = self.solve_WDP(
                self.inferred_bids_raised, self.MIP_parameters,
                verbose=1)  # find the efficient allocation with respect to the raised bids
            self.number_of_mips_solved['WDP'] += 1
            end = datetime.now()
            self.total_time_wdp_mip += (end - start).total_seconds()
            efficiency_raised_bids = self.calculate_efficiency_of_allocation(allocation=allocation_raised_bids,
                                                                             allocation_scw=true_value_raised_bids)
        ...
        if self.mechanism_parameters['calculate_profit_max_bids']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                ...
                start = datetime.now()
                allocation_profit_max_bids, true_value_profit_max_bids, inferred_value_profit_max_bids, details = self.solve_WDP(
                    profit_max_bids, self.MIP_parameters,
                    verbose=1)  # find the efficient allocation with respect to the (raised) bids of the clock round + profit-max bids
                self.number_of_mips_solved['WDP'] += 1
                end = datetime.now()
                self.total_time_wdp_mip += (end - start).total_seconds()
                efficiency_profit_max_bids = self.calculate_efficiency_of_allocation(
                    allocation=allocation_profit_max_bids, allocation_scw=true_value_profit_max_bids)
        ...
        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                ...
                start = datetime.now()
                allocation_profit_max_bids_unraised, true_value_profit_max_bids_unraised, inferred_value_profit_max_bids_unraised, details = self.solve_WDP(
                    profit_max_bids_unraised, self.MIP_parameters,
                    verbose=1)  # find the efficient allocation with respect to the (raised) bids of the clock round + profit-max bids
                self.number_of_mips_solved['WDP'] += 1
                end = datetime.now()
                self.total_time_wdp_mip += (end - start).total_seconds()
                efficiency_profit_max_bids_unraised = self.calculate_efficiency_of_allocation(
                    allocation=allocation_profit_max_bids_unraised, allocation_scw=true_value_profit_max_bids_unraised)

                self.final_allocation_profit_max_unraised[
                    number_of_profit_max_bids] = allocation_profit_max_bids_unraised
                self.final_allocation_scw_profit_max_unraised[
                    number_of_profit_max_bids] = true_value_profit_max_bids_unraised
                self.final_allocation_efficiency_profit_max_unraised[
                    number_of_profit_max_bids] = efficiency_profit_max_bids_unraised
    ...
    def calculate_allocation_value(self, allocation):
        ...
        for bidder_name in allocation.keys():
            ...
            good_ids = self.SATS_auction_instance.get_good_ids()
            if not self.generic_domain:
                allocated_bundle_indices = allocation[bidder_name]['good_ids']
                allocated_bundle = [good_ids[j] for j in allocated_bundle_indices]
                for j in allocated_bundle_indices:
                    allocated_bundle[j] = 1
            else:
                allocated_bundle = allocation[bidder_name]['allocated_bundle']
                allocated_bundle_indices = np.where(allocated_bundle >= 1)[
                    0]  # NOTE: commpletely irrelevant field for generic domains

            true_bundle_value = self.SATS_auction_instance.calculate_value(key_to_int(bidder_name), allocated_bundle)

            logging.info(
                f'True value of {bidder_name}\'s bundle is {true_bundle_value} while inferred value is {inferred_value}')
    ...
    def solve_WDP(self,
        ...
        if not self.generic_domain:
            ...
            elicited_bundle_value_pairs = [np.concatenate((bids[0], np.asarray(bids[1]).reshape(-1, 1)), axis=1) for
                                           bidder, bids in
                                           elicited_bids.items()]  # transform self.elicited_bids into format for WDP class
            wdp = MLCA_DQ_WDP_GENERIC(
                bids=elicited_bundle_value_pairs,
                MIP_parameters=MIP_parameters,
                capacity_generic_items=self.good_capacities,
                SATS_auction_instance=self.SATS_auction_instance  # 新增参数
            )
            wdp.initialize_mip(verbose=0)
            wdp.solve_mip(verbose)
            inferred_value = wdp.Mip.objective_value  # the value of the allocation with respect to the inferred bids, needed for VCG payments
            ...
            logging.debug(f'Allocation details: {allocation}')

        allocation_value, allocation_transformed = self.calculate_allocation_value(
            allocation)  # calculate the true value of the allocation (not the inferred value)
        ...
        logging.debug('WDP求解输入 bids 样例:\n%s',
                      json.dumps(
                          {bidder: {
                              'bundles': bids[0].tolist(),  # 输出bundle
                              'values': bids[1].tolist()  # 输出value
        ...
        if verbose == 1:
            for key, value in allocation.items():
                logging.debug('%s %s', key, value[
                    'good_ids'])  # note: the efficiency is wrt. the true value of this allocation, not the inferred values.
    ...
    def calculate_clearing_allocation(self, demand_vector, price_vector, is_final_allocation):
        ...
        for bidder_id in range(len(self.bidder_names)):
            allocation[f'Bidder_{bidder_id}'] = {
                'good_ids': [i for i in range(len(demand_vector[bidder_id])) if demand_vector[bidder_id][i] == 1]}
            allocation[f'Bidder_{bidder_id}']['value'] = np.dot(demand_vector[bidder_id],
                                                                price_vector)  # just for bookkeeping, not used
        ...
        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                self.efficiency_per_iteration_profit_max_unraised[number_of_profit_max_bids][
                    self.mlca_iteration] = efficiency
        ...
        if current_clock_round in self.mechanism_parameters['calculate_profit_max_bids_specific_rounds']:
            ...
            self.MIP_relative_gap_per_iteration_profit_max_specific_round[current_clock_round] = [0 for _ in range(
                self.mechanism_parameters['profit_max_grid'][-1] + 1)]
            self.MIP_time_per_iteration_profit_max_specific_round[current_clock_round] = [0 for _ in range(
                self.mechanism_parameters['profit_max_grid'][-1] + 1)]
            self.MIP_unsatisfied_constraints_per_iteration_profit_max_specific_round[current_clock_round] = [0 for _ in
                                                                                                             range(
                                                                                                                 self.mechanism_parameters[
                                                                                                                     'profit_max_grid'][
                                                                                                                     -1] + 1)]
        ...
        if is_final_allocation:
            ...
            if self.mechanism_parameters['calculate_raised_bids']:
                ...
                # also set clearing payment (note they equal 'inferred values calculated above')
                for bidder_name in self.bidder_names:
                    self.clearing_payments_raised_bids[bidder_name] = self.final_allocation_raised_bids[bidder_name][
                        'inferred_value']
            ...
            if self.mechanism_parameters['calculate_profit_max_bids']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    ...
                    self.relative_revenue_profit_max[number_of_profit_max_bids] = self.revenue_profit_max[
                                                                                      number_of_profit_max_bids] / self.SATS_auction_instance_scw
                    # also set clearing payment (note they equal 'inferred values calculated above')
                    for bidder_name in self.bidder_names:
                        self.clearing_payments_profit_max[number_of_profit_max_bids][bidder_name] = \
                        self.final_allocation_profit_max[number_of_profit_max_bids][bidder_name]['inferred_value']
            ...
            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    ...
                    self.relative_revenue_profit_max_unraised[number_of_profit_max_bids] = \
                    self.revenue_profit_max_unraised[number_of_profit_max_bids] / self.SATS_auction_instance_scw
                    # also set clearing payment (note they equal 'inferred values calculated above')
                    for bidder_name in self.bidder_names:
                        self.clearing_payments_profit_max_unraised[number_of_profit_max_bids][bidder_name] = \
                        self.final_allocation_profit_max_unraised[number_of_profit_max_bids][bidder_name][
                            'inferred_value']
    ...
    def calculate_vcg_payments(self):
        ...
        if self.mechanism_parameters['calculate_profit_max_bids']:
            self.marginal_economies_allocations_profit_max = {j: {} for j in
                                                              self.mechanism_parameters['profit_max_grid']}
            self.marginal_economies_scw_profit_max = {j: {} for j in self.mechanism_parameters['profit_max_grid']}
            self.marginal_economies_inferred_scw_profit_max = {j: {} for j in
                                                               self.mechanism_parameters['profit_max_grid']}

        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            self.marginal_economies_allocations_profit_max_unraised = {j: {} for j in
                                                                       self.mechanism_parameters['profit_max_grid']}
            self.marginal_economies_scw_profit_max_unraised = {j: {} for j in
                                                               self.mechanism_parameters['profit_max_grid']}
            self.marginal_economies_inferred_scw_profit_max_unraised = {j: {} for j in
                                                                        self.mechanism_parameters['profit_max_grid']}
        ...
        for bidder_id in self.SATS_auction_instance.get_bidder_ids():
            ...
            marginal_inferred_bids[f'Bidder_{bidder_id}'][0] = np.array(
                [[0 for i in range(len(self.SATS_auction_instance.get_good_ids()))]])

            # calculate the marginal allocation and its apparent social welfare
            marginal_allocation, marginal_true_value, marginal_inferred_value, details = self.solve_WDP(
                marginal_inferred_bids, self.MIP_parameters, verbose=1)
            self.number_of_mips_solved['WDP'] += 1
            ...
            if self.mechanism_parameters['calculate_raised_bids']:
                ...
                marginal_inferred_bids_raised[f'Bidder_{bidder_id}'][0] = np.array(
                    [[0 for i in range(len(self.SATS_auction_instance.get_good_ids()))]])

                # calculate the marginal allocation and its social welfare for the raised bids
                marginal_allocation_raised_bids, marginal_true_value_raised_bids, marginal_inferred_value_raised_bids, details = self.solve_WDP(
                    marginal_inferred_bids_raised, self.MIP_parameters, verbose=1)
                self.number_of_mips_solved['WDP'] += 1
                ...
                self.marginal_economies_inferred_scw_raised_bids[
                    f'Bidder_{bidder_id}'] = marginal_inferred_value_raised_bids

            # if we are using profit max, we also need to calculate the marginal allocation for the profit max bids
            if self.mechanism_parameters['calculate_profit_max_bids']:

                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    marginal_inferred_bids_profit_max = copy.deepcopy(
                        self.profit_max_bids_combined[number_of_profit_max_bids])
                    marginal_inferred_bids_profit_max[f'Bidder_{bidder_id}'][1] = np.array([0])
                    marginal_inferred_bids_profit_max[f'Bidder_{bidder_id}'][0] = np.array(
                        [[0 for i in range(len(self.SATS_auction_instance.get_good_ids()))]])

                    # calculate the marginal allocation and its social welfare for the raised bids
                    marginal_allocation_profit_max_bids, marginal_true_value_profit_max_bids, marginal_inferred_value_profit_max_bids, details = self.solve_WDP(
                        marginal_inferred_bids_profit_max, self.MIP_parameters, verbose=1)
                    self.number_of_mips_solved['WDP'] += 1

                    self.marginal_economies_allocations_profit_max[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_allocation_profit_max_bids
                    self.marginal_economies_scw_profit_max[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_true_value_profit_max_bids
                    self.marginal_economies_inferred_scw_profit_max[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_inferred_value_profit_max_bids

            # same for profit max without raising the clock bids.
            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:

                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    marginal_inferred_bids_profit_max_unraised = copy.deepcopy(
                        self.profit_max_bids_combined_unraised[number_of_profit_max_bids])
                    marginal_inferred_bids_profit_max_unraised[f'Bidder_{bidder_id}'][1] = np.array([0])
                    marginal_inferred_bids_profit_max_unraised[f'Bidder_{bidder_id}'][0] = np.array(
                        [[0 for i in range(len(self.SATS_auction_instance.get_good_ids()))]])

                    # calculate the marginal allocation and its social welfare for the raised bids
                    marginal_allocation_profit_max_bids_unraised, marginal_true_value_profit_max_bids_unraised, marginal_inferred_value_profit_max_bids_unraised, details = self.solve_WDP(
                        marginal_inferred_bids_profit_max, self.MIP_parameters, verbose=1)
                    self.number_of_mips_solved['WDP'] += 1

                    self.marginal_economies_allocations_profit_max_unraised[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_allocation_profit_max_bids_unraised
                    self.marginal_economies_scw_profit_max_unraised[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_true_value_profit_max_bids_unraised
                    self.marginal_economies_inferred_scw_profit_max_unraised[number_of_profit_max_bids][
                        f'Bidder_{bidder_id}'] = marginal_inferred_value_profit_max_bids_unraised
        ...
        for bidder in self.bidder_names:
            ...
            main_economy_apparent_scw_without_bidder = sum(
                [final_allocation[i]['inferred_value'] for i in final_allocation.keys() if i != bidder])
            self.vcg_payments[bidder] = round(apparent_scw_marginal_economy - main_economy_apparent_scw_without_bidder,
                                              2)
            logging.info(
                f'Payment {bidder}: {apparent_scw_marginal_economy:.2f} - {main_economy_apparent_scw_without_bidder:.2f}  =  {self.vcg_payments[bidder]:.2f}')
            print(
                f'Payment {bidder}: {apparent_scw_marginal_economy:.2f} - {main_economy_apparent_scw_without_bidder:.2f}  =  {self.vcg_payments[bidder]:.2f}')
            ...
            if self.mechanism_parameters['calculate_raised_bids']:
                ...
                main_economy_apparent_scw_without_bidder_raised_bids = sum(
                    [final_allocation_raised_bids[i]['inferred_value'] for i in final_allocation_raised_bids.keys() if
                     i != bidder])
                self.vcg_payments_raised_bids[bidder] = round(
                    apparent_scw_marginal_economy_raised_bids - main_economy_apparent_scw_without_bidder_raised_bids, 2)
                logging.info(
                    f'Payment raised bids {bidder}: {apparent_scw_marginal_economy_raised_bids:.2f} - {main_economy_apparent_scw_without_bidder_raised_bids:.2f}  =  {self.vcg_payments_raised_bids[bidder]:.2f}')

            # if we are using profit max, we also need to calculate the VCG terms for the profit max bids
            if self.mechanism_parameters['calculate_profit_max_bids']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    apparent_scw_marginal_economy_profit_max = \
                    self.marginal_economies_inferred_scw_profit_max[number_of_profit_max_bids][bidder]
                    main_economy_apparent_scw_without_bidder_profit_max = sum(
                        [final_allocation_profit_max[number_of_profit_max_bids][i]['inferred_value'] for i in
                         self.marginal_economies_allocations_profit_max[number_of_profit_max_bids].keys() if
                         i != bidder])
                    self.vcg_payments_profit_max[number_of_profit_max_bids][bidder] = round(
                        apparent_scw_marginal_economy_profit_max - main_economy_apparent_scw_without_bidder_profit_max,
                        2)
                    logging.info(
                        f'Payment profit max ({number_of_profit_max_bids} bids) bidder {bidder}: {apparent_scw_marginal_economy_profit_max:.2f} - {main_economy_apparent_scw_without_bidder_profit_max:.2f}  =  {self.vcg_payments_profit_max[number_of_profit_max_bids][bidder]:.2f}')

                    # same logic for profit max with unraised clock bids
            if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
                for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                    apparent_scw_marginal_economy_profit_max_unraised = \
                    self.marginal_economies_inferred_scw_profit_max_unraised[number_of_profit_max_bids][bidder]
                    main_economy_apparent_scw_without_bidder_profit_max_unraised = sum(
                        [final_allocation_profit_max_unraised[number_of_profit_max_bids][i]['inferred_value'] for i in
                         self.marginal_economies_allocations_profit_max_unraised[number_of_profit_max_bids].keys() if
                         i != bidder])
                    self.vcg_payments_profit_max_unraised[number_of_profit_max_bids][bidder] = round(
                        apparent_scw_marginal_economy_profit_max_unraised - main_economy_apparent_scw_without_bidder_profit_max_unraised,
                        2)
                    logging.info(
                        f'Payment profit max UNRAISED ({number_of_profit_max_bids} bids) bidder {bidder}: {apparent_scw_marginal_economy_profit_max_unraised:.2f} - {main_economy_apparent_scw_without_bidder_profit_max_unraised:.2f}  =  {self.vcg_payments_profit_max[number_of_profit_max_bids][bidder]:.2f}')
        ...
        if self.mechanism_parameters['calculate_raised_bids']:
            ...
            self.relative_revenue_raised_bids = self.revenue_raised_bids / self.SATS_auction_instance_scw
            logging.info(
                'Revenue raised bids: {} | {}% of SCW in efficient allocation\n'.format(self.revenue_raised_bids,
                                                                                        self.relative_revenue_raised_bids * 100))

        if self.mechanism_parameters['calculate_profit_max_bids']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                self.revenue_profit_max[number_of_profit_max_bids] = sum(
                    [self.vcg_payments_profit_max[number_of_profit_max_bids][i] for i in self.bidder_names])
                self.relative_revenue_profit_max[number_of_profit_max_bids] = self.revenue_profit_max[
                                                                                  number_of_profit_max_bids] / self.SATS_auction_instance_scw
                logging.info('Revenue profit max ({} bids): {} | {}% of SCW in efficient allocation\n'.format(
                    number_of_profit_max_bids, self.revenue_profit_max[number_of_profit_max_bids],
                    self.relative_revenue_profit_max[number_of_profit_max_bids] * 100))

        if self.mechanism_parameters['calculate_profit_max_bids_unraised']:
            for number_of_profit_max_bids in self.mechanism_parameters['profit_max_grid']:
                self.revenue_profit_max_unraised[number_of_profit_max_bids] = sum(
                    [self.vcg_payments_profit_max_unraised[number_of_profit_max_bids][i] for i in self.bidder_names])
                self.relative_revenue_profit_max_unraised[number_of_profit_max_bids] = self.revenue_profit_max_unraised[
                                                                                           number_of_profit_max_bids] / self.SATS_auction_instance_scw
