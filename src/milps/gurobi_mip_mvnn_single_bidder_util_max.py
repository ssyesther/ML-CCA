    ...
    def generate_mip(self,
        ...
        for (i, layer) in enumerate(self.model.layers):
            for (j, weight) in enumerate(layer.weight.data):
                ...
                # print(f"y_variables[{i}] length: {len(self.y_variables[i])}")
                assert len(weight) <= len(self.y_variables[i]), \
                    f"Weight dim {len(weight)} exceeds variables count {len(self.y_variables[i])}"
                # CASE 1 -> REMOVAL:
