...
class MVNN_GENERIC(nn.Module):
    ...
    def __init__(self,
        ...
        print(f"[初始化验证] 接收到的input_dim: {input_dim}")  
        print(f"[初始化验证] capacity_generic_goods类型: {type(capacity_generic_goods)}")
        ...
        capacity_values = list(self.capacity_generic_goods.values()) if isinstance(self.capacity_generic_goods, dict) else self.capacity_generic_goods
        print(f"[DEBUG] capacity_values类型: {type(capacity_values)} 长度: {len(capacity_values)}")
        
        expanded_capacity = []
        for cap in capacity_values:
            expanded_capacity.extend([cap] * cap)
        print(f"[DEBUG] 扩展后容量列表长度: {len(expanded_capacity)}")
        
        generic_trafo_layer_weight = np.diag(1/np.repeat(capacity_values, list(capacity_values)))
        print(f"[DEBUG] 权重矩阵形状: {generic_trafo_layer_weight.shape}")

        print(f"[权重矩阵验证] 输入维度: {input_dim} 容量参数长度: {len(capacity_values)}")
        generic_trafo_layer_weight = np.diag(1 / np.repeat(capacity_values, list(capacity_values)))
        print(f"[权重矩阵验证] 权重矩阵形状: {generic_trafo_layer_weight.shape}")
